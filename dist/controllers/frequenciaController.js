...
exports.frequenciaController = {
    ...
    async registrarChamada(req, res) {
        ...
        const promessas = presencas.map(async (p) => {
            const existente = await prisma_1.prisma.frequencia.findFirst({
                where: { alunoId: p.alunoId, data: dataFormatada, disciplinaId }
            });
            if (existente) {
                return prisma_1.prisma.frequencia.update({
                    where: { id: existente.id },
                    data: { presente: p.presente }
                });
            }
            else {
                return prisma_1.prisma.frequencia.create({
                    data: {
                        data: dataFormatada,
                        presente: p.presente,
    ...
    async prepararChamada(req, res) {
        ...
        if (diaSemana === 0)
            throw new errorHandler_1.AppError('Não há aulas agendadas para domingo', 400);
        ...
        const aulaAtiva = await prisma_1.prisma.gradeHoraria.findFirst({
            where: {
                escolaId,
                diaSemana,
        ...
        if (!aulaAtiva) {
            throw new errorHandler_1.AppError('Nenhuma aula agendada para este horário ou você não é o professor desta disciplina.', 404);
        }
    ...
    async salvarChamadaInteligente(req, res) {
        ...
        const professoresDistintos = await prisma_1.prisma.turmaDisciplina.groupBy({
            by: ['professorId'],
            where: { turmaId }
        });
        const isFundamental1 = professoresDistintos.length === 1;
        // 2. Executar gravação
        await prisma_1.prisma.$transaction(presencas.map((p) => {
            return prisma_1.prisma.frequencia.upsert({
                where: {
    ...
    async listarChamadaRealizada(req, res) {
        const { turmaId, disciplinaId, data } = req.query;
        const escolaId = req.user?.escolaId;
        const dataFormatada = new Date(data);
        dataFormatada.setUTCHours(0, 0, 0, 0);
        const historico = await prisma_1.prisma.frequencia.findMany({
            where: {
                escolaId,
                turmaId: turmaId,
                disciplinaId: disciplinaId,
...
//# sourceMappingURL=frequenciaController.js.map