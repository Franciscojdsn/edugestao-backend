...
exports.eventoController = {
    async create(req, res) {
        ...
        if (dados.turmaId) {
            const turma = await prisma_1.prisma.turma.findFirst({ where: (0, prismaHelpers_1.withEscolaId)({ id: dados.turmaId }) });
            if (!turma)
        ...
        if (dados.dataFim && new Date(dados.dataFim) < new Date(dados.dataInicio)) {
            throw new errorHandler_1.AppError('Data de fim não pode ser anterior à data de início', 400);
        }
    ...
    async show(req, res) {
        ...
        const evento = await prisma_1.prisma.evento.findFirst({
    ...
    async update(req, res) {
        ...
        if (dados.dataFim)
            updateData.dataFim = new Date(dados.dataFim);
        if (dados.horaInicio !== undefined)
            updateData.horaInicio = dados.horaInicio;
        if (dados.horaFim !== undefined)
            updateData.horaFim = dados.horaFim;
        if (dados.diaLetivo !== undefined)
            updateData.diaLetivo = dados.diaLetivo;
        if (dados.publico !== undefined)
            updateData.publico = dados.publico;
        if (dados.turmaId !== undefined)
            updateData.turmaId = dados.turmaId;
        const atualizado = await prisma_1.prisma.evento.update({ where: { id: idFormatado }, data: updateData });
    ...
    async delete(req, res) {
        ...
        if (!evento)
            throw new errorHandler_1.AppError('Evento não encontrado', 404);
        await prisma_1.prisma.evento.delete({ where: { id: idFormatado } });
        return res.status(204).send();
    },
    async proximosEventos(req, res) {
        const hoje = new Date();
