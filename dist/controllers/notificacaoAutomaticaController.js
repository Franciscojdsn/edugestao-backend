...
exports.notificacaoAutomaticaController = void 0;
const prisma_1 = require("../config/prisma");
const errorHandler_1 = require("../middlewares/errorHandler");
// Lógica de escalonamento baseada em dias de atraso
function getEscalation(diasVencidos) {
    if (diasVencidos <= 5) {
        return {
            nivel: 'LEMBRETE',
            prioridade: 'NORMAL',
...
exports.notificacaoAutomaticaController = {
    // Lista quem está devendo para visualização do gestor
    async verificarPendencias(req, res) {
        const escolaId = req.user?.escolaId;
        const hoje = new Date();
        const boletosVencidos = await prisma_1.prisma.boletos.findMany({
            where: {
    ...
    async dispararLembretes(req, res) {
        const escolaId = req.user?.escolaId;
        const hoje = new Date();
        hoje.setHours(0, 0, 0, 0);
        const boletos = await prisma_1.prisma.boletos.findMany({
        ...
        for (const bol of boletos) {
            ...
            const jaNotificado = await prisma_1.prisma.comunicado.findFirst({
                where: { alunoId: bol.alunoId, titulo: { contains: 'Vencimento' }, createdAt: { gte: hoje } }
            });
            if (!jaNotificado) {
                await prisma_1.prisma.comunicado.create({
                    data: {
    ...
    async resumoPendencias(req, res) {
        const escolaId = req.user?.escolaId;
        const { alunoId, turmaId } = req.query;
        const hoje = new Date();
        let whereAluno = { escolaId, deletedAt: null };
        if (alunoId)
            whereAluno.id = alunoId;
        if (turmaId)
    ...
    async escalonarPagamento(req, res) {
        ...
        const hoje = new Date();
        hoje.setHours(0, 0, 0, 0);
        const diasVencidos = Math.max(0, Math.floor((hoje.getTime() - pagamento.dataVencimento.getTime()) / (1000 * 60 * 60 * 24)));
        // Força próximo nível
        let nivel;
        if (diasVencidos <= 5)
            nivel = 'AVISO';
...
//# sourceMappingURL=notificacaoAutomaticaController.js.map