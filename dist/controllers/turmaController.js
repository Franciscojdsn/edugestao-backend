...
exports.turmaController = {
    ...
    async list(req, res) {
        ...
        const { anoLetivo, turno, busca } = req.query;
        ...
        const [turmas, total] = await Promise.all([
            ...
            prisma_1.prisma.turma.count({ where }),
        ]);
        return res.json({
            data: turmas,
            meta: {
    ...
    async create(req, res) {
        ...
        const turmaExistente = await prisma_1.prisma.turma.findFirst({
            where: {
        ...
        if (turmaExistente) {
            throw new errorHandler_1.AppError('Já existe uma turma com este nome para o ano letivo informado', 400);
        }
    ...
    async update(req, res) {
        ...
        const { professorResponsavelId, ano: _ano, // Descartado: campo legado que causa o erro PrismaClientValidationError
        id: _id, // Descartado: o ID vem dos parâmetros da URL
        escolaId: _escolaId, // Descartado: escolaId vem do req.user
        createdAt: _createdAt, updatedAt: _updatedAt, _count: _count, ...dados } = req.body;
        const escolaId = req.user?.escolaId;
        ...
        if (!turmaExistente) {
            throw new errorHandler_1.AppError('Turma não encontrada ou acesso negado', 404);
        }
        ...
        if (dados.nome || dados.anoLetivo) {
            ...
            const conflito = await prisma_1.prisma.turma.findFirst({
                where: {
                    escolaId,
            ...
            if (conflito) {
                throw new errorHandler_1.AppError('Já existe outra turma com este nome para este ano letivo', 400);
            }
        }
    ...
    async delete(req, res) {
        ...
        if (turma._count.alunos > 0) {
            throw new errorHandler_1.AppError(`Ação negada: Existem ${turma._count.alunos} aluno(s) nesta turma. Transfira-os antes de deletar.`, 400);
        }
        // Deletar
        await prisma_1.prisma.turma.delete({
            where: { id },
        });
