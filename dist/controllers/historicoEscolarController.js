...
exports.historicoEscolarController = {
    /**
    ...
    async gerar(req, res) {
        ...
        const { anoInicio, anoFim, incluirFrequencia = true, incluirFinanceiro = false } = req.query;
        ...
        let whereNotas = { alunoId: idFormatado };
        if (anoInicio || anoFim) {
            whereNotas.anoLetivo = {};
            if (anoInicio)
                whereNotas.anoLetivo.gte = Number(anoInicio);
            if (anoFim)
                whereNotas.anoLetivo.lte = Number(anoFim);
        }
        // Buscar notas
        ...
        const historicoAcademico = Object.keys(notasPorAno).map(ano => {
            const disciplinas = Object.values(notasPorAno[ano]).map((disc) => {
                const soma = disc.notas.reduce((s, n) => s + n.valor, 0);
        ...
        if (incluirFrequencia) {
            let whereFreq = { alunoId: idFormatado };
            if (anoInicio || anoFim) {
                whereFreq.data = {};
                if (anoInicio)
                    whereFreq.data.gte = new Date(Number(anoInicio), 0, 1);
                if (anoFim)
                    whereFreq.data.lte = new Date(Number(anoFim), 11, 31);
            }
            const frequencias = await prisma_1.prisma.frequencia.findMany({
                where: whereFreq,
            });
        ...
        if (incluirFinanceiro) {
            ...
            const pagamentos = await prisma_1.prisma.boletos.findMany({
                where: { alunoId: idFormatado },
                select: {
                    referencia: true,
                    valorTotal: true,
        ...
        // Atividades extra
        const atividadesExtra = await prisma_1.prisma.alunoAtividadeExtra.findMany({
            where: {
                alunoId: idFormatado,
                ativo: true,
            },
    ...
    async resumo(req, res) {
        ...
        const [totalNotas, mediaGeral, totalFrequencias, totalComunicados] = await Promise.all([
            prisma_1.prisma.nota.count({ where: { alunoId: idFormatado } }),
            prisma_1.prisma.nota.aggregate({
                where: { alunoId: idFormatado },
                _avg: { valor: true },
            }),
            prisma_1.prisma.frequencia.count({ where: { alunoId: idFormatado } }),
            prisma_1.prisma.comunicado.count({ where: { alunoId: idFormatado } }),
        ]);
        const presencas = await prisma_1.prisma.frequencia.count({
