...
exports.relatorioController = {
    ...
    async financeiro(req, res) {
        ...
        const resumoBoletos = boletos.reduce((acc, boleto) => {
        ...
        const fluxoCaixa = lancamentos.reduce((acc, lanc) => {
            ...
            if (lanc.tipo === 'ENTRADA') {
                ...
                if (lanc.status === 'PAGO')
                    acc.entradasRealizadas += valor;
            }
            else {
                acc.saidasPrevistas += valor;
                if (lanc.status === 'PAGO')
                    acc.saidasRealizadas += valor;
            }
            return acc;
    ...
    async pedagogico(req, res) {
        ...
        const resumoPorDisciplina = notas.reduce((acc, nota) => {
            ...
            if (valorNota < acc[key].menorNota)
                acc[key].menorNota = valorNota;
            if (valorNota > acc[key].maiorNota)
                acc[key].maiorNota = valorNota;
        ...
        const relatorio = Object.values(resumoPorDisciplina).map((item) => ({
            ...item,
