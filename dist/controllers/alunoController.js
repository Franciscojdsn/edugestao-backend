...
exports.alunoController = {
     ...
     */
    async list(req, res) {
        const { page = 1, limit = 10, turmaId, turno, busca, status // Recebe o status da query string
         } = req.query;
        ...
        const [alunos, totalAlunos, totalInadimplentes] = await Promise.all([
            prisma_1.prisma.aluno.findMany({
            ...
            prisma_1.prisma.aluno.count({ where: (0, prismaHelpers_1.withEscolaId)(where) }),
    ...
    async show(req, res) {
        ...
        return res.json({
            ...aluno,
            status: aluno.status || statusDerivado
        });
    },
    /**
    ...
    async create(req, res) {
        ...
        const matriculaExiste = await prisma_1.prisma.aluno.findFirst({
            where: (0, prismaHelpers_1.withEscolaId)({
                numeroMatricula: dados.numeroMatricula,
            }),
        });
        if (matriculaExiste) {
            throw new errorHandler_1.AppError('Número de matrícula já está em uso', 400);
        }
    ...
    async update(req, res) {
        ...
        const { nome, dataNascimento, genero, cpf, numeroSus, planoSaude, hospital, matriculaPlano, turmaId, turno, 
        // Dados de endereço (mapeados para a tabela relacionada)
        cep, logradouro, // Mapeia para 'rua' no schema
        numero, complemento, bairro, cidade, estado, } = req.body;
        const idFormatado = Array.isArray(id) ? id[0] : id;
        // 2. Verificar se aluno existe e pertence à escola (Multi-tenancy)
        const alunoExistente = await prisma_1.prisma.aluno.findFirst({
        ...
        const aluno = await prisma_1.prisma.aluno.update({
