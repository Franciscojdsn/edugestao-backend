...
exports.enderecoController = {
    ...
    async show(req, res) {
        ...
        const endereco = await prisma_1.prisma.endereco.findUnique({
    ...
    async update(req, res) {
        ...
        const dados = req.body;
        const idFormatado = Array.isArray(id) ? id[0] : id;
        const enderecoExistente = await prisma_1.prisma.endereco.findUnique({
            where: { id: idFormatado },
        });
        if (!enderecoExistente)
            throw new errorHandler_1.AppError('Endereço não encontrado', 404);
        const endereco = await prisma_1.prisma.endereco.update({
            where: { id: idFormatado },
            data: dados,
        });
        return res.json(endereco);
    ...
    async delete(req, res) {
        ...
        if (total > 0) {
            throw new errorHandler_1.AppError(`Não é possível deletar endereço vinculado a ${total} registro(s)`, 400);
        }
        await prisma_1.prisma.endereco.delete({ where: { id: idFormatado } });
        return res.status(204).send();
    },
    async buscarPorCep(req, res) {
