...
exports.matriculaController = {
    ...
    async iniciar(req, res) {
        ...
        if (!escolaId)
            throw new errorHandler_1.AppError('Escola não identificada', 403);
        ...
        while (matriculaExiste && tentativas < 5) {
            ...
            const conflito = await prisma_1.prisma.aluno.findFirst({
     ...
     */
    async adicionarResponsavel(req, res) {
        ...
        const { usarEnderecoDoAluno, enderecoId: enderecoFrontId, endereco, // Objeto de endereço vindo do form (ignorado se for herança)
        ...dadosResponsavel } = req.body;
        // 3. Busca da Matrícula e Aluno (Isolamento de Dados)
        const matricula = await prisma_1.prisma.matricula.findFirst({
            where: { id: matriculaId.toString(), escolaId },
            include: { aluno: { select: { enderecoId: true } } }
        });
        if (!matricula)
            throw new errorHandler_1.AppError('Matrícula não localizada.', 404);
        // 4. Lógica de Decisão do Endereço (UUID v4)
        let enderecoFinalId = null;
    ...
    async finalizar(req, res) {
        const { matriculaId } = req.params;
        const { valorMatricula, descontoMatricula, // Descomente assim que rodar o prisma db push
        valorMensalidadeBase, descontoMensalidade, atividadesExtrasIds, diaVencimento, qtdParcelas, responsavelFinanceiroId } = req.body;
        const escolaId = req.user?.escolaId;
        if (!escolaId)
            throw new errorHandler_1.AppError('Tenant (escolaId) não identificado no token.', 403);
        const matriculaRascunho = await prisma_1.prisma.matricula.findFirst({
            where: { id: matriculaId.toString(), escolaId },
            include: { aluno: true }
        });
        if (!matriculaRascunho)
            throw new errorHandler_1.AppError('Matrícula não encontrada.', 404);
        ...
        const resultado = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            if (atividadesExtrasIds && atividadesExtrasIds.length > 0) {
                const vinculosExtras = atividadesExtrasIds.map((extraId) => ({
                    alunoId: matriculaRascunho.alunoId,
            ...
            await tx.boletos.createMany({ data: boletosData });
            return {
    ...
    async list(req, res) {
        ...
        const [matriculas, total] = await Promise.all([
            prisma_1.prisma.matricula.findMany({
                where,
                skip,
                take: Number(limit),
                include: {
            ...
            prisma_1.prisma.matricula.count({ where }),
        ]);
        return res.json({
            data: matriculas,
    ...
    async atualizarStatus(req, res) {
        ...
        if (!matricula)
            throw new errorHandler_1.AppError('Matrícula não encontrada', 404);
        const atualizada = await prisma_1.prisma.matricula.update({
    ...
    async cancelar(req, res) {
        ...
        await prisma_1.prisma.aluno.update({
            where: { id: matricula.alunoId },
            data: { deletedAt: new Date() },
        });
        // Cancelar matrícula
        await prisma_1.prisma.matricula.update({
            where: { id: idFormatado },
